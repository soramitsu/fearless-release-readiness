...
check_files_and_markers() {
  ...
  require_pattern "$SERVICE_DIR/docs/production-deployment.md" 'PASSKEY_ALLOWED_ORIGINS=https://fearlesswallet\.io,https://backup\.fearlesswallet\.io' "passkey challenge service production origins docs"
	  ...
	  require_pattern "$SERVICE_DIR/docs/production-deployment.md" 'PASSKEY_DEPLOYMENT_GH_BIN' "passkey challenge service authenticated gh authority docs"
	  require_pattern "$SERVICE_DIR/docs/production-deployment.md" 'exact recorded GitHub Actions run' "passkey challenge service exact publication-run authentication docs"
	  require_pattern "$SERVICE_DIR/docs/production-deployment.md" 'gh attestation verify --bundle' "passkey challenge service exact attestation-bundle verification docs"
	  require_pattern "$SERVICE_DIR/docs/production-deployment.md" 'blocked diagnostic mode does not contact GitHub' "passkey challenge service blocked no-network docs"
	  ...
	  require_pattern "$SERVICE_DIR/docs/release-checklist.md" 'PASSKEY_DEPLOYMENT_GH_BIN' "passkey challenge service authenticated gh authority checklist"
	  require_pattern "$SERVICE_DIR/docs/release-checklist.md" 'exact recorded Actions run and exact attestation ID' "passkey challenge service exact publication provenance checklist"
	  require_pattern "$SERVICE_DIR/docs/release-checklist.md" 'gh attestation verify --bundle' "passkey challenge service exact attestation-bundle checklist"
	  require_pattern "$SERVICE_DIR/docs/release-checklist.md" 'blocked diagnostic mode must make no GitHub call' "passkey challenge service blocked no-network checklist"
	  ...
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" 'PASSKEY_DEPLOYMENT_GH_BIN' "passkey challenge service explicit gh authority gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" 'function authenticatePublicationRecord' "passkey challenge service authenticated GitHub publication-run gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" 'actions/runs/\$\{runId\}' "passkey challenge service exact Actions run API gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" 'attestations/\$\{record\.imageDigest\}' "passkey challenge service digest-bound attestation collection API gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" 'run\.repository\.full_name === EXPECTED_PUBLICATION_REPOSITORY' "passkey challenge service publication repository identity gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" 'run\.path === EXPECTED_IMAGE_PUBLICATION_WORKFLOW' "passkey challenge service publication workflow identity gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" 'run\.event === EXPECTED_PUBLICATION_EVENT' "passkey challenge service workflow-dispatch event gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" 'run\.head_sha === record\.deployedCommit' "passkey challenge service publication source commit gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" 'imageProvenanceAttestationUrl download must contain exactly one Sigstore bundle' "passkey challenge service exact downloaded attestation bundle structure gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" "'--bundle'" "passkey challenge service exact attestation bundle verification gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" "'--signer-workflow'" "passkey challenge service attestation signer-workflow gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" "'--source-digest'" "passkey challenge service attestation source-digest gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" "'--source-ref'" "passkey challenge service attestation source-ref gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" "'--deny-self-hosted-runners'" "passkey challenge service hosted-runner provenance gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" "GH_HOST: 'github\.com'" "passkey challenge service canonical GitHub host gate"
	  require_pattern "$SERVICE_DIR/scripts/audit-deployment-evidence.sh" 'readyEvidenceRequired && !process\.exitCode' "passkey challenge service post-validation provenance boundary"
	  ...
	  require_pattern "$SERVICE_DIR/scripts/test-deployment-evidence-audit.sh" 'blocked evidence makes zero GitHub calls' "passkey challenge service blocked no-GitHub-call test"
	  require_pattern "$SERVICE_DIR/scripts/test-deployment-evidence-audit.sh" 'ready evidence authenticates exact GitHub Actions run and attestation bundle' "passkey challenge service authenticated publication positive test"
	  require_pattern "$SERVICE_DIR/scripts/test-deployment-evidence-audit.sh" 'authenticated Actions run drift is rejected' "passkey challenge service publication-run drift matrix"
	  require_pattern "$SERVICE_DIR/scripts/test-deployment-evidence-audit.sh" 'missing malformed or incomplete exact attestation bundle is rejected' "passkey challenge service exact attestation bundle negative matrix"
	  require_pattern "$SERVICE_DIR/scripts/test-deployment-evidence-audit.sh" 'attestation verification failure is fail closed' "passkey challenge service provenance verification failure test"
  ...
  require_pattern "$SERVICE_DIR/src/store.js" '^const CREDENTIAL_STORE_SCHEMA_VERSION = 4;' "passkey challenge service credential store schema-v4"
  require_pattern "$SERVICE_DIR/src/store.js" '^const MIGRATABLE_CREDENTIAL_STORE_SCHEMA_VERSION = 3;' "passkey challenge service preserved schema-v3 migration"
  require_pattern "$SERVICE_DIR/src/store.js" 'validateCredentialOwnerIndex' "passkey challenge service credential owner index integrity"
