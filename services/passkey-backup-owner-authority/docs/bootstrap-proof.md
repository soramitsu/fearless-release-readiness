...
The platform attestation nonce is SHA-256 of ASCII `FP_OWNER_BOOTSTRAP_APP_V1` plus zero byte, then length-prefixed wallet message, scheme, normalized public-key bytes and signature bytes. It is encoded as canonical unpadded base64url. The server-configured Android app identity is `android:<packageName>:<lowercase Play signing certificate SHA-256 hex>`; the iOS identity is `ios:<TeamID>:<bundleID>`. Every allowed Android WebAuthn origin must encode that same Play signing certificate digest. A trusted server adapter must verify the actual Play Integrity token or App Attest object and return **exactly** that nonce, platform and app identity. Truthy flags or echoes supplied by a client are insufficient. Omission keeps bootstrap unavailable and the production HTTP factory still rejects construction.

The optional Android adapter in `src/play-integrity-admission.js` accepts only a server-owned package name, Play app-signing certificate digest, allowed Play version-code list and OAuth access-token callback. It sends the opaque client token in Google's canonical `integrityToken` request field to the fixed `decodeIntegrityToken` endpoint with redirect following disabled and a five-second deadline. It admits only a standard-token response whose request hash equals this signed-wallet nonce, whose request and app package match configuration, whose timestamp is no older than two minutes or more than 30 seconds in the future, and whose version code and certificate match the configured release. It also requires `PLAY_RECOGNIZED`, `LICENSED` and `MEETS_DEVICE_INTEGRITY` and refuses Play Console testing overrides; errors, missing verdicts, classic-token nonce fields, redirects and oversized responses deny without reflecting token material. Google recommends checking request details before verdicts and documents the server-side decode and verdict fields: [REST discovery schema](https://playintegrity.googleapis.com/$discovery/rest?version=v1), [standard requests](https://developer.android.com/google/play/integrity/standard), [verdicts](https://developer.android.com/google/play/integrity/verdicts). The callback must acquire a service-account token with the `playintegrity` scope; it is not derived from the mobile request. Its test uses a simulated Google HTTP response, so the exact Play-distributed certificate, release version, real Google decode, supported-device policy and native request-hash interoperability still require operator provisioning and device tests. iOS App Attest has no corresponding adapter yet.

The one-use ceremony is claimed before asynchronous verification. A bad proof may burn that ceremony but cannot create or reassign an owner. A native client must keep local PRF output and backup keys out of this request entirely. Production admission still requires live Google and Apple attestation validation, SR25519 support for relevant wallets, historical-owner migration, native vectors, independent review and replacement-device tests.
